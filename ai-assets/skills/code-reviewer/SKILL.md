...
name: code-reviewer
description: >
  Frontend code review skill. Systematically reviews React/TypeScript code covering component design,
  type safety, Hook usage, performance issues, error handling, and code standards.
  Triggers when user says "review code", "check the code", "any code issues",
  "review PR", "look at changes".
  Even if user doesn't explicitly say "review", proactively use this skill when evaluating existing code quality,
  identifying potential issues, or optimizing code structure.
