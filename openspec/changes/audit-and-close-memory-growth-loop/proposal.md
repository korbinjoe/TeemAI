...
- **No agent workspace contains a `memory/` folder.** `~/.teemai/agents/<id>/` only holds the static `IDENTITY.md` / `SOUL.md` / `TOOLS.md` (`HEARTBEAT.md` and `BOOT.md` for a subset). The BOOT/HEARTBEAT scripts that say "log to today's memory/YYYY-MM-DD.md" never have a directory to write into.
